  ...
  return (
    <div className="flex">
      <div className="bg-gray-900 p-4 text-white ">
        <div className="mt-4">
          {Object.entries(groupedBlocks).map(([key, value]) => (
            <div key={key} className="mb-2">
              <h4 className="text-sm text-gray-400 my-4">{key}</h4>
              <div className="flex gap-4 flex-wrap">
                {value.map((item, index) => (
                  <button
                    key={index}
                    className={`relative w-8 h-8 rounded flex items-center justify-center transition-colors text-gray-400 ${item.color} ${item.borderColor} ${item.hoverBorderColor} group hoverEffect`}
                  >
                    {<item.icon size="20" />}
                    {/* Tooltip */}
                    <span
